...
export const CertificateListAdmin: React.FC = () => {
  ...
  const loadCertificates = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const studentIds = [...new Set((data.content || []).map((c: Certificate) => c.studentId).filter((id: any) => id))];
  ...
  const updateStatus = async (certificateId: string, newStatus: string) => {
    try {
      ...
      const res = await apiCall(`/api/certificates/status`, {
        method: 'PATCH',
        body: JSON.stringify({
          id: certificateId,
          status: newStatus
  ...
  const deleteCertificate = async (certificateId: string) => {
    ...
    try {
      const res = await apiCall(`/api/certificates/${certificateId}`, {
        method: 'DELETE'
      });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Admin panel for managing all certificates</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Template Filter</label>
              <input
                type="text"
            ...
            <div className="flex items-end">
              <button
                onClick={() => { setFilterStatus(''); setFilterTemplate(''); setCurrentPage(0); }}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  return (
                    <tr key={cert.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{cert.serialNo}</div>
                          <div className="text-sm text-gray-500">ID: {cert.certId || cert.id}</div>
                          <div className="text-sm text-gray-500">Template: {cert.templateId}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          {student?.xepLoai && (
                            <div className="text-sm text-gray-500">Grade: {student.xepLoai}</div>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          ...
                          className={`text-sm rounded-full px-2 py-1 border ${getStatusColor(cert.status)} focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50`}
                        >
                          <option value="ACTIVE">Active</option>
                          <option value="EXPIRED">Expired</option>
                          <option value="REVOKED">Revoked</option>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        {cert.expireAt && (
                          <div>Expires: {new Date(cert.expireAt).toLocaleDateString()}</div>
                        )}
                      </td>
          ...
          {certificates.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-gray-500">Try adjusting your filters or create new certificates.</p>
            </div>
          )}
        </div>

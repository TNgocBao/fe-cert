...
export const CertificateSignFormStaff: React.FC = () => {
  ...
  const loadStudentsInCourse = async (courseCode: string) => {
    ...
    try {
      ...
      const response = await apiCall(`http://localhost:8080/api/results/course/${courseCode}/students`);
      ...
      if (response.ok) {
        ...
        if (data && data.success) {
          ...
          const enrichedStudents = studentsWithResults.map((result: any) => {
            const studentInfo = allStudents.find(s => s.studentCode === result.studentCode);
            return {
              ...studentInfo,
              ...result,
              id: studentInfo?.id || 0
  ...
  const handleSignCertificates = async () => {
    ...
    try {
      ...
      formDataToSend.append("p12File", formData.p12File, formData.p12File.name);
      ...
      formDataToSend.append("courseCode", formData.courseCode); // Course code ở root level
      ...
      const studentsForBackend = selectedStudents.map(student => ({
        ...
        // KHÔNG có courseCode ở student level
      }));
      ...
      studentsForBackend.forEach((student, index) => {
        formDataToSend.append(`students[${index}].studentCode`, student.studentCode);
        formDataToSend.append(`students[${index}].name`, student.name);
        formDataToSend.append(`students[${index}].dob`, student.dob);
        formDataToSend.append(`students[${index}].majorName`, student.majorName);
        formDataToSend.append(`students[${index}].timeStudied`, student.timeStudied);
      ...
      const response = await fetch('http://localhost:8080/api/v1/requests/sign-certificate', {
        method: 'POST',
        body: formDataToSend,
      });
      ...
      setError("Lỗi khi ký chứng chỉ: " + "tồn tại đã ký");
    } finally {
      setSigning(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  style={{ borderColor: colors.border, boxShadow: `0 0 0 2px ${colors.accent}` }}
                  required
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange("keystorePass", e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2"
                  style={{ borderColor: colors.border, boxShadow: `0 0 0 2px ${colors.accent}` }}
                  required
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6" style={{ borderColor: colors.accent }}>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {selectedStudents.map((student) => (
                  <div key={student.id} className="flex items-center justify-between p-3 border rounded-lg" style={{ borderColor: colors.border }}>
                    <div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6" style={{ borderColor: colors.border }}>
          <h2 className="text-xl font-semibold mb-4" style={{ color: colors.text }}>
            {selectedCourse ? `Sinh Viên Trong Khóa Học (${studentsInCourse.length})` : 'Chọn Sinh Viên'}
          </h2>
              ...
              <div className="max-h-96 overflow-y-auto">
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: isSelected ? colors.primary + "20" : "white",
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                              <span className="text-xs px-2 py-1 rounded" style={{ backgroundColor: colors.primary, color: colors.accent }}>
                                {student.studentCode}
                              </span>
                              <span className="text-xs text-gray-600">{student.className}</span>
                              {student.score && <span className="text-xs text-gray-600">Điểm: {student.score}</span>}
                              {student.grade && <span className="text-xs text-gray-600">Xếp loại: {student.grade}</span>}
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {filteredStudents.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm" style={{ color: colors.textLight }}>
                      {searchTerm ? "Thử tìm kiếm với từ khóa khác." : "Không có sinh viên nào trong khóa học này."}
                    </p>
                  </div>
                )}
              </div>

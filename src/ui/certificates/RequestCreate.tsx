...
console.log(user?.fullName);
  ...
  const loadTemplates = async () => {
    try {
      ...
      // For now, skip template loading since endpoint might not exist
      // const res = await apiCall('/api/certificates/templates');
      // if (res.ok) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 text-lg">Gửi yêu cầu cấp chứng chỉ của bạn một cách nhanh chóng và dễ dàng</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <label className="relative">
                  ...
                  <div className="p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 peer-checked:border-purple-500 peer-checked:bg-purple-50 transition-all">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="font-medium text-gray-900">Chứng Chỉ Mới</div>
                        <div className="text-sm text-gray-500">Yêu cầu cấp chứng chỉ tốt nghiệp lần đầu</div>
                      </div>
                    </div>
                  </div>
                </label>
                ...
                <label className="relative">
                  ...
                  <div className="p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 peer-checked:border-purple-500 peer-checked:bg-purple-50 transition-all">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="font-medium text-gray-900">Gia Hạn Chứng Chỉ</div>
                        <div className="text-sm text-gray-500">Gia hạn chứng chỉ đã hết hạn</div>
                      </div>
                    </div>
                  </div>
                </label>
                ...
                <label className="relative">
                  ...
                  <div className="p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-purple-300 peer-checked:border-purple-500 peer-checked:bg-purple-50 transition-all">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="font-medium text-gray-900">Thay Thế Chứng Chỉ</div>
                        <div className="text-sm text-gray-500">Cấp lại chứng chỉ bị mất hoặc hỏng</div>
                      </div>
                    </div>
                  </div>
                </label>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h3 className="text-sm font-medium text-blue-800 mb-3">Thông Tin Sinh Viên</h3>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-600">Tên:</span>
                  <span className="ml-2 font-medium">{user?.fullName || user?.name || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-600">Mã SV:</span>
                  <span className="ml-2 font-medium">{user?.studentCode || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-600">Xếp loại:</span>
                  <span className="ml-2 font-medium">{user?.xepLoai || 'N/A'}</span>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-6">
              <div className="flex items-start">
                ...
                <div className="ml-4 flex-1">
                  <h3 className="text-lg font-semibold text-purple-800 mb-2">Chứng Chỉ Sẽ Được Tạo</h3>
                  <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Tên sinh viên:</span>
                        <span className="font-medium text-purple-700">{user?.fullName || user?.name || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Mã sinh viên:</span>
                        <span className="font-medium text-purple-700">{user?.studentCode || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Loại chứng chỉ:</span>
                        <span className="font-medium text-purple-700">
                          {form.requestType === 'NEW_CERTIFICATE' ? 'Chứng Chỉ Mới' :
                           form.requestType === 'RENEWAL' ? 'Gia Hạn' : 'Thay Thế'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Ngày cấp dự kiến:</span>
                        <span className="font-medium text-purple-700">{new Date().toLocaleDateString('vi-VN')}</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-sm text-purple-600 mt-3">
                    Chứng chỉ sẽ được tạo tự động với thông tin của bạn sau khi được duyệt.
                  </p>
                </div>
              </div>
            </div>
